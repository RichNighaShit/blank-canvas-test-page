  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        if (message.toLowerCase().includes('user already registered') || message.toLowerCase().includes('already exists')) {
    ...
    <div className="min-h-screen bg-gradient-hero flex items-center justify-center p-4">
      <Card className="w-full max-w-md shadow-elegant">
        <CardHeader className="text-center space-y-4">
          ...
          <div>
            <CardTitle className="text-2xl font-bold">Welcome to DripMuse</CardTitle>
            <CardDescription>
                ...
                {signupError && (
                  <div className="text-red-500 text-sm">
                    ...
                    {signupError.includes('already registered') && (
                      <div>
                        <span> </span>
                        <a href="#" onClick={() => navigate('?tab=signin')} className="underline text-blue-600">Sign in</a>
                        <span> or </span>
                        <a href="https://{YOUR_SUPABASE_PROJECT_DOMAIN}.supabase.co/auth/reset-password" target="_blank" rel="noopener noreferrer" className="underline text-blue-600">reset your password</a>.
                      </div>
                    )}
                  </div>
                )}
