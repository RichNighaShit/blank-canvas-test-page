...
function toggleSelection(array: string[], item: string, max?: number): string[] {
  ...
  const handlePhotoAnalysis = (analysisResult: any) => {
    ...
    if (analysisResult?.imageUrl) {
      setForm(prev => ({
        ...
        favorite_colors: analysisResult.colors && Array.isArray(analysisResult.colors) 
    ...
    <div className="min-h-screen bg-gradient-hero py-8">
      <div className="container mx-auto px-4 max-w-2xl">
        <div className="mb-8 text-center">
          ...
          <p className="text-muted-foreground">Update your style preferences and personal info</p>
        </div>
        <Card className="shadow-elegant">
          <CardHeader>
            <CardTitle>Edit Profile</CardTitle>
            <CardDescription>Make changes to your style profile below</CardDescription>
          </CardHeader>
          <CardContent>
            <form className="space-y-6" onSubmit={handleSubmit}>
              <div className="flex flex-col md:flex-row gap-6">
                <div className="flex-1 space-y-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={value => handleChange('gender_identity', value)}
                    >
                      <SelectTrigger>
                ...
                <div className="flex-1 space-y-4">
                  ...
                  {form.face_photo_url && (
                    <img src={form.face_photo_url} alt="Profile" className="w-24 h-24 rounded-full object-cover mt-2 border" />
                  )}
                </div>
              </div>
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-3">
                  {styleOptions.map(style => (
                    <Button
                      key={style.id}
                      type="button"
                      variant={form.preferred_style === style.id ? "default" : "outline"}
                ...
                <div className="grid grid-cols-6 gap-3 mt-3">
                  {colorOptions.map(color => (
                    <Button
                      ...
                      onClick={() => handleChange('favorite_colors', toggleSelection(form.favorite_colors, color.id, 6))}
                    >
                ...
                <div className="grid grid-cols-2 gap-3 mt-3">
                  {goalOptions.map(goal => (
                    <Button
                      key={goal.id}
                      type="button"
                      variant={form.goals.includes(goal.id) ? "default" : "outline"}
                      className="h-auto p-4 flex items-center gap-3 text-left justify-start"
                      onClick={() => handleChange('goals', toggleSelection(form.goals, goal.id, 2))}
                    >
              ...
              <div className="flex justify-end gap-4 mt-6">
                ...
                <Button type="submit" className="shadow-button" disabled={isSaving}>
                  {isSaving ? (
                    <>
