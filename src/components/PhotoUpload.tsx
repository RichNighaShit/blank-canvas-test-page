...
export const PhotoUpload = ({ onAnalysisComplete }: PhotoUploadProps) => {
  ...
  const [autoFitPreviewUrl, setAutoFitPreviewUrl] = useState<string | null>(null);
  ...
  const getCroppedImg = async (imageSrc: string, cropPixels: any) => {
    ...
    return new Promise<File>((resolve) => {
      canvas.toBlob((blob) => {
        if (blob) {
          const file = new File([blob], fileRef.current?.name || 'cropped.jpg', { type: 'image/jpeg' });
          resolve(file);
        }
      }, 'image/jpeg');
    });
  ...
  const createAutoFitPreview = async (userImgUrl: string) => {
    try {
      ...
      await Promise.all([
        ...
        new Promise((resolve) => { mannequinImg.onload = resolve; })
      ]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70">
          <div className="bg-white p-4 rounded shadow-lg max-w-lg w-full flex flex-col items-center">
            <div className="relative w-72 h-96 bg-gray-200 flex flex-col items-center justify-center">
              ...
              {autoFitPreviewUrl && (
                <img src={autoFitPreviewUrl} alt="Auto-Fit Preview" className="mt-4 rounded shadow" style={{ maxHeight: '320px' }} />
              )}
            </div>
            <div className="flex gap-4 mt-4">
              <Button onClick={async () => {
                ...
                const file = new File([blob], 'autoFitPreview.png', { type: 'image/png' });
                // Upload to Supabase
                const fileName = `${user.id}/autoFitPreview.png`;
                const { error: uploadError } = await supabase.storage.from('user-photos').upload(fileName, file, { upsert: true });
                if (!uploadError) {
              ...
              <Button variant="outline" onClick={() => { setShowAutoFit(false); setAutoFitNotice(false); setAutoFitPreviewUrl(null); }} disabled={isAnalyzing}>
                Cancel
              </Button>
            </div>
            {autoFitNotice && (
              <div className="mt-4 text-xs text-center text-red-600 bg-red-50 p-2 rounded">
                Try-on preview is AI-assisted based on your face. For best results, upload a torso photo.
              </div>
            )}
          </div>
        </div>
