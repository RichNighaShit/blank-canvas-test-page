...
export const WardrobeAnalytics = () => {
  ...
  const analyzeWardrobe = async () => {
    ...
    try {
      ...
      // Analyze styles
      const styleDistribution = items.reduce(
        (acc, item) => {
          if (item.style) {
      ...
      const seasonalCoverage = items.reduce(
        (acc, item) => {
          if (item.season) {
      ...
      const occasionCoverage = items.reduce(
        (acc, item) => {
          if (item.occasion) {
      ...
      const gapsInWardrobe = identifyWardrobeGaps(
        categoryBreakdown,
        occasionCoverage,
        seasonalCoverage,
      );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="shadow-card">
          ...
          <CardContent>
            <div className="text-2xl font-bold capitalize">
              {Object.entries(stats.styleDistribution).sort(
                ([, a], [, b]) => b - a,
              )[0]?.[0] || "None"}
            </div>
          ...
          <CardContent className="space-y-4">
            {["spring", "summer", "fall", "winter"].map((season) => (
              <div key={season} className="space-y-2">
                ...
                <Progress
                  value={
                    ((stats.seasonalCoverage[season] || 0) / stats.totalItems) *
                    100
                  }
                  className="h-2"
                />
