    ...
    try {
      ...
      if (!items || items.length === 0) {
        generatedInsights.push({
          ...
          description:
            "Upload photos of your clothes to get personalized insights and recommendations.",
          action: "Add Items",
          priority: "high",
        });
      } else {
        ...
        const occasionCount = items.reduce(
          (acc, item) => {
            if (item.occasion) {
        ...
        if ((occasionCount.formal || 0) < 2) {
          generatedInsights.push({
            ...
            description:
              "You have limited formal wear options. Consider adding a few formal pieces for special occasions.",
            action: "Shop Formal",
            priority: "medium",
          });
        }
        ...
        if ((occasionCount.work || 0) < 3) {
          generatedInsights.push({
            ...
            description:
              "Your professional wardrobe could use more options. Aim for 5-7 work-appropriate outfits.",
        ...
        const versatileItems = items.filter(
          (item) => item.occasion && item.occasion.length >= 3,
        );
        ...
        if (versatileItems.length < items.length * 0.3) {
          generatedInsights.push({
            ...
            description:
              "Many of your items work for only one occasion. Look for pieces that can be dressed up or down.",
        ...
        if (Object.keys(categoryCount).length >= 5) {
          generatedInsights.push({
            ...
            description:
              "Your wardrobe covers multiple clothing categories, giving you flexibility in outfit creation.",
            priority: "low",
          });
        }
      }
  ...
  const handleActionClick = (insight: WardrobeInsight) => {
    ...
    switch (insight.action) {
      ...
      case "Explore Colors":
        ...
        window.open(
          "https://www.google.com/search?tbm=isch&q=clothing+color+palette+inspiration",
          "_blank",
        );
        break;
      case "Find Versatile Pieces":
        ...
        window.open(
          "https://www.google.com/search?tbm=shop&q=versatile+clothing+pieces+capsule+wardrobe",
          "_blank",
        );
        break;
      default:
      ...
      {insights.length === 0 && (
        <Card className="shadow-card">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground">
              Add more items to your wardrobe to get personalized insights and
              recommendations.
            </p>
          </CardContent>
        </Card>
